...
class JSONFormater {
    static String format(String jsonStr){
        try {
    ...
    private static int outputNextLineAndRightMove(int space, char ch, ByteArrayOutputStream out) throws IOException {
        //换行
        outputNewline(out);
        //向右缩进
        out.write(getBlankingStringBytes(space));
    ...
    private static int outputBackAndLeftMove(int space, char ch, ByteArrayOutputStream out) throws IOException {
        space -= 2;
        out.write(getBlankingStringBytes(space));
        out.write(ch);
        return space;
